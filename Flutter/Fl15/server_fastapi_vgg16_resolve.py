...
# Change from GET to POST and add file parameter for prediction
@app.post('/sample')
async def sample_prediction(file: UploadFile = File(...)):
    try:
        ...
        # Make prediction using the model (assuming prediction_model can handle the raw image data)
        result = vgg16_prediction_model.prediction_model(vgg16_model, image)
